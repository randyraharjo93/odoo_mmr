...
class CrmTeam(models.Model):
    ...
    team_invoiced = fields.Integer(string='Team Invoiced', compute="compute_team_invoiced", readonly=True)
    ...
    def compute_team_target(self):
        ...
        childs += self
        for team in childs:
            total += team.invoiced_target
        self.team_target = total
        return

    @api.one
    def compute_team_invoiced(self):
        total = 0
        childs = self.get_childs(self.id)
        childs += self
        for team in childs:
            total += team.invoiced
        self.team_invoiced = total
        return
